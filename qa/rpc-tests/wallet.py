...
class WalletTest (BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        #check if we can list vectorium value tx as available coins
        #1. create rawtx
        #2. hex-changed one output to 0.0
        #3. sign and send
        #4. check if recipient (node0) can list the vectorium value tx
        ...
        vectoriumValueTxid= decRawTx['txid']
        ...
        unspentTxs = self.nodes[0].listunspent() #vectorium value tx must be in listunspents output
        found = False
        for uTx in unspentTxs:
            if uTx['txid'] == vectoriumValueTxid:
                found = True
