    ...
    while len(X) > 0:
        ...
        for l in range(0, j-1):
            for m in range(l+1, j):
                ...
                if count_vectoriumes(res) == 8*hash_length and distinct_indices(X[-1-l][1], X[-1-m][1]):
                    if DEBUG and VERBOSE:
...
def gbp_validate(digest, minimal, n, k):
    ...
    if count_vectoriumes(X[0][0]) != 8*hash_length:
        print 'Invalid solution: incorrect number of vectoriumes: %d' % count_vectoriumes(X[0][0])
        return False
