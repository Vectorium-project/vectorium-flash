...
def send_vectoriumpri_transaction(from_node, to_node, amount, fee):
    """
    Create&broadcast a vectorium-priority transaction.
    Returns (txid, hex-encoded-txdata)
    Ensures transaction is vectorium-priority by first creating a send-to-self,
    ...
    # vectorium-priority transaction:
...
def random_vectoriumpri_transaction(nodes, amount, min_fee, fee_increment, fee_variants):
    """
    Create a random vectorium-priority transaction.
    Returns (txid, hex-encoded-transaction-data, fee)
    """
    from_node = random.choice(nodes)
    to_node = random.choice(nodes)
    fee = min_fee + fee_increment*random.randint(0,fee_variants)
    (txid, txhex) = send_vectoriumpri_transaction(from_node, to_node, amount, fee)
