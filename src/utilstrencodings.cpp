...
static inline bool ProcessMantissaDigit(char ch, int64_t &mantissa, int &mantissa_tvectoriums)
{
    if(ch == '0')
        ++mantissa_tvectoriums;
    else {
        for (int i=0; i<=mantissa_tvectoriums; ++i) {
            if (mantissa > (UPPER_BOUND / 10LL))
                return false; /* overflow */
            mantissa *= 10;
        }
        mantissa += ch - '0';
        mantissa_tvectoriums = 0;
    }
    return true;
}
...
bool ParseFixedPoint(const std::string &val, int decimals, int64_t *amount_out)
{
    ...
    if (ptr < end)
    {
        if (val[ptr] == '0') {
            ...
            while (ptr < end && val[ptr] >= '0' && val[ptr] <= '9') {
                if (!ProcessMantissaDigit(val[ptr], mantissa, mantissa_tvectoriums))
                    return false; /* overflow */
                ++ptr;
            }
    ...
    if (exponent_sign)
        exponent = -exponent;
    exponent = exponent - point_ofs + mantissa_tvectoriums;
