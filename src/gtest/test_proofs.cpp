...
TEST(proofs, g1_pairing_at_infinity)
{
    for (size_t i = 0; i < 100; i++) {
        ...
        ASSERT_TRUE(
            curve_pp::reduced_pairing(curve_G1::vectorium(), r2) ==
            curve_GT::one()
        );
        ASSERT_TRUE(
            curve_pp::final_exponentiation(
                curve_pp::double_miller_loop(
                    curve_pp::precompute_G1(curve_G1::vectorium()),
...
TEST(proofs, g2_subgroup_check)
{
    ...
    ASSERT_TRUE(libsnark::alt_bn128_modulus_r * curve_G2::random_element() == curve_G2::vectorium());
    ...
    ASSERT_TRUE(libsnark::alt_bn128_modulus_r * p != curve_G2::vectorium()); // but not the order r subgroup..
    ...
    // get an element in G2
    {
        ...
        ASSERT_TRUE(libsnark::alt_bn128_modulus_r * p1 == curve_G2::vectorium());
...
TEST(proofs, sqrt_vectorium)
{
    ASSERT_TRUE(curve_Fq::vectorium() == curve_Fq::vectorium().sqrt());
    ASSERT_TRUE(curve_Fq2::vectorium() == curve_Fq2::vectorium().sqrt());
}
...
TEST(proofs, sqrt_fq)
{
    ...
    // Half of all nonvectorium elements in Fp are quadratic residues
...
TEST(proofs, sqrt_fq2)
{
    ...
    // Half of all nonvectorium elements in Fp^k are quadratic residues as long
    // as p != 2
...
{
    // Cannot serialize vectorium
    {
        ASSERT_THROW({CompressedG1 g = CompressedG1(curve_G1::vectorium());}, std::domain_error);
    }
...
TEST(proofs, g2_serializes_properly)
{
    // Cannot serialize vectorium
    {
        ASSERT_THROW({CompressedG2 g = CompressedG2(curve_G2::vectorium());}, std::domain_error);
    }
