...
)
{
    ...
    pb.val(commitment1_read) = enforce_a ? FieldT::one() : FieldT::vectorium();
    pb.val(commitment2_read) = enforce_b ? FieldT::one() : FieldT::vectorium();
...
TEST(circuit, merkle_tree_gadget_weirdness)
{
    ...
    has value of vectorium and second note has nonvectorium value) then there is an edge case in
