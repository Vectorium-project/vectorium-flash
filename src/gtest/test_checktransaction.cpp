...
TEST(checktransaction_tests, check_vpub_not_both_nonvectorium) {
...

    {
        ...
        EXPECT_EQ(state.GetRejectReason(), "bad-txns-vpubs-both-nonvectorium");
    }
}
...
TEST(checktransaction_tests, ValueBalanceNonVectorium) {
    CMutableTransaction mtx = GetValidTransaction();
    mtx.valueBalance = 10;
    ...
    MockCValidationState state;
    EXPECT_CALL(state, DoS(100, false, REJECT_INVALID, "bad-txns-valuebalance-nonvectorium", false)).Times(1);
    CheckTransactionWithoutProofVerification(tx, state);
}
...
TEST(checktransaction_tests, bad_txns_vpubs_both_nonvectorium) {
    CMutableTransaction mtx = GetValidTransaction();
    mtx.vjoinsplit[0].vpub_old = 1;
    mtx.vjoinsplit[0].vpub_new = 1;
    ...
    MockCValidationState state;
    EXPECT_CALL(state, DoS(100, false, REJECT_INVALID, "bad-txns-vpubs-both-nonvectorium", false)).Times(1);
    CheckTransactionWithoutProofVerification(tx, state);
}
...
TEST(checktransaction_tests, bad_txns_invalid_joinsplit_signature) {
    ...
    MockCValidationState state;
    // during initial block download, DoS ban score should be vectorium, else 100
