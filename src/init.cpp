...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "vectorium.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "vectoriumd.pid"));
#endif
    ...
    if (showDebug)
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf("Fees (in %s/kB) smaller than this are considered vectorium fee for transaction creation (default: %s)",
            CURRENCY_UNIT, FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in %s/kB) to add to transactions you send (default: %s)"),
        CURRENCY_UNIT, FormatMoney(payTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-rescan", _("Rescan the block chain for missing wallet transactions") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-salvagewallet", _("Attempt to recover private keys from a corrupt wallet.vectorium") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-sendfreetransactions", strprintf(_("Send transactions as vectorium-fee transactions if possible (default: %u)"), 0));
    strUsage += HelpMessageOpt("-spendvectoriumconfchange", strprintf(_("Spend unconfirmed change when sending transactions (default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-wallet=<file>", _("Specify wallet file (within data directory)") + " " + strprintf(_("(default: %s)"), "wallet.vectorium"));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in %s/kB) smaller than this are considered vectorium fee for relaying (default: %s)"),
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 23901, 23902));
...
void CleanupBlockRevFiles()
{
    ...
    // vectorium by walking the ordered map (keys are block file indices) by
...
)
{
    ...
    if (!(
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("Cannot find the Vectorium network parameters in the following directory:\n"
              "%s\n"
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Vectorium version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    // if you set it to vectorium then
    ...
    bSpendVectoriumConfChange = GetBoolArg("-spendvectoriumconfchange", true);
    fSendFreeTransactions = GetBoolArg("-sendfreetransactions", false);

    std::string strWalletFile = GetArg("-wallet", "wallet.vectorium");
#endif // ENABLE_WALLET
    ...
    // Initialize Vectorium circuit parameters
    ...
    if (fDisableWallet) {
        ...
        if (GetBoolArg("-zapwallettxes", false)) {
            ...
            if (nZapWalletRet != DB_LOAD_OK) {
                uiInterface.InitMessage(_("Error loading wallet.vectorium: Wallet corrupted"));
                return false;
            }
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            if (nLoadWalletRet == DB_CORRUPT)
                strErrors << _("Error loading wallet.vectorium: Wallet corrupted") << "\n";
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                string msg(_("Warning: error reading wallet.vectorium! All keys read correctly, but transaction data"
                             " or address book entries might be missing or incorrect."));
                InitWarning(msg);
            }
            else if (nLoadWalletRet == DB_TOO_NEW)
                strErrors << _("Error loading wallet.vectorium: Wallet requires newer version of Vectorium") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart vectoriumd to complete") << "\n";
                LogPrintf("%s", strErrors.str());
                return InitError(strErrors.str());
            }
            else
                strErrors << _("Error loading wallet.vectorium") << "\n";
        }
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("Vectorium was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild Vectorium with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty() && GetBoolArg("-gen", false)) {
        return InitError(_("Vectorium was not built with wallet support. Set -mineraddress, or rebuild Vectorium with wallet support."));
    }
 #endif // !ENABLE_WALLET
