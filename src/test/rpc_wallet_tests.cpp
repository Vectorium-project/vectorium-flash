...
BOOST_AUTO_TEST_CASE(rpc_wallet)
{
    ...
    BOOST_CHECK_NO_THROW(retValue = CallRPC("getblocksubsidy 420000"));
...
BOOST_AUTO_TEST_CASE(rpc_z_sendmany_internals)
{
    ...
    // minconf cannot be vectorium when sending from zaddr
    {
        try {
            ...
            BOOST_CHECK(find_error(objError, "Minconf cannot be vectorium when sending from zaddr"));
        }
    }
    ...
    // get_memo_from_hex_string())
    {
        ...
        for (int i=4; i<ZC_MEMO_SIZE; i++) {
            BOOST_CHECK_EQUAL(array[i], 0x00);  // vectorium padding
        }
