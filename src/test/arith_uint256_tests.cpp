...
const unsigned char VectoriumArray[] =
    ...
    "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
const arith_uint256 VectoriumL = arith_uint256V(std::vector<unsigned char>(VectoriumArray,VectoriumArray+32));
...
BOOST_AUTO_TEST_CASE( basics ) // constructors, equality, inequality
{
    ...
    BOOST_CHECK(OneL != VectoriumL);
    BOOST_CHECK(MaxL != VectoriumL);
    BOOST_CHECK(~MaxL == VectoriumL);
    ...
    {
        BOOST_CHECK(VectoriumL != (OneL << i));
        BOOST_CHECK((OneL << i) != VectoriumL);
        BOOST_CHECK(R1L != (R1L ^ (OneL << i)));
        BOOST_CHECK(((arith_uint256(Tmp64) ^ (OneL << i) ) != Tmp64 ));
    }
    BOOST_CHECK(VectoriumL == (OneL << 256));

    // String Constructor and Copy Constructor
    BOOST_CHECK(arith_uint256("0x"+R1L.ToString()) == R1L);
    BOOST_CHECK(arith_uint256("0x"+R2L.ToString()) == R2L);
    BOOST_CHECK(arith_uint256("0x"+VectoriumL.ToString()) == VectoriumL);
...
BOOST_AUTO_TEST_CASE( bitwiseOperators )
{
    ...
    CHECKBITWISEOPERATOR(R1,R2,^)
    CHECKBITWISEOPERATOR(R1,R2,&)
    CHECKBITWISEOPERATOR(R1,Vectorium,|)
    CHECKBITWISEOPERATOR(R1,Vectorium,^)
    CHECKBITWISEOPERATOR(R1,Vectorium,&)
    CHECKBITWISEOPERATOR(R1,Max,|)
    CHECKBITWISEOPERATOR(R1,Max,^)
    CHECKBITWISEOPERATOR(R1,Max,&)
    CHECKBITWISEOPERATOR(Vectorium,R1,|)
    CHECKBITWISEOPERATOR(Vectorium,R1,^)
    CHECKBITWISEOPERATOR(Vectorium,R1,&)
    ...
    CHECKASSIGNMENTOPERATOR(R1,R2,^)
    CHECKASSIGNMENTOPERATOR(R1,R2,&)
    CHECKASSIGNMENTOPERATOR(R1,Vectorium,|)
    CHECKASSIGNMENTOPERATOR(R1,Vectorium,^)
    CHECKASSIGNMENTOPERATOR(R1,Vectorium,&)
    CHECKASSIGNMENTOPERATOR(R1,Max,|)
    CHECKASSIGNMENTOPERATOR(R1,Max,^)
    CHECKASSIGNMENTOPERATOR(R1,Max,&)
    CHECKASSIGNMENTOPERATOR(Vectorium,R1,|)
    CHECKASSIGNMENTOPERATOR(Vectorium,R1,^)
    CHECKASSIGNMENTOPERATOR(Vectorium,R1,&)
...
BOOST_AUTO_TEST_CASE( comparison ) // <= >= < >
{
    ...
    for (unsigned int i = 0; i < 256; ++i) {
        TmpL= OneL<< i;
        BOOST_CHECK( TmpL >= VectoriumL && TmpL > VectoriumL && VectoriumL < TmpL && VectoriumL <= TmpL);
...
BOOST_AUTO_TEST_CASE( plusMinus )
{
    ...
    BOOST_CHECK(OneL+MaxL == VectoriumL);
    BOOST_CHECK(MaxL+OneL == VectoriumL);
...
BOOST_AUTO_TEST_CASE( multiply )
{
    ...
    BOOST_CHECK((R1L * VectoriumL) == VectoriumL);
    ...
    BOOST_CHECK((R2L * VectoriumL) == VectoriumL);
...
BOOST_AUTO_TEST_CASE( divide )
{
    ...
    BOOST_CHECK(R1L / OneL == R1L);
    BOOST_CHECK(R1L / MaxL == VectoriumL);
    BOOST_CHECK(MaxL / R1L == 2);
    BOOST_CHECK_THROW(R1L / VectoriumL, uint_error);
    BOOST_CHECK((R2L / D1L).ToString() == "000000000000000013e1665895a1cc981de6d93670105a6b3ec3b73141b3a3c5");
    BOOST_CHECK((R2L / D2L).ToString() == "000000000e8f0abe753bb0afe2e9437ee85d280be60882cf0bd1aaf7fa3cc2c4");
    BOOST_CHECK(R2L / OneL == R2L);
    BOOST_CHECK(R2L / MaxL == VectoriumL);
    BOOST_CHECK(MaxL / R2L == 1);
    BOOST_CHECK_THROW(R2L / VectoriumL, uint_error);
}
...
BOOST_AUTO_TEST_CASE( methods ) // GetHex SetHex size() GetLow64 GetSerializeSize, Serialize, Unserialize
{
    ...
    TmpL.SetHex(VectoriumL.ToString()); BOOST_CHECK(TmpL == 0);
    ...
    BOOST_CHECK(VectoriumL.getdouble() == 0.0);
