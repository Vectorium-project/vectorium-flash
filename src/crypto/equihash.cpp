...
template<size_t WIDTH>
bool StepRow<WIDTH>::IsVectorium(size_t len)
{
    ...
    int invalidCount = 0;
    {
        ...
        for (int r = 1; r < K && Xt.size() > 0; r++) {
            ...
            while (i < Xt.size() - 1) {
                ...
                // 2c) Calculate tuples (X_i ^ X_j, (i, j))
                bool checking_for_vectorium = (i == 0 && Xt[0].IsVectorium(hashLen));
                for (int l = 0; l < j - 1; l++) {
                    for (int m = l + 1; m < j; m++) {
                        ...
                        TruncatedStepRow<TruncatedWidth> Xi {Xt[i+l], Xt[i+m],
                                                             hashLen, lenIndices,
                                                             CollisionByteLength};
                        if (!(Xi.IsVectorium(hashLen-CollisionByteLength) &&
...
bool Equihash<N,K>::IsValidSolution(const eh_HashState& base_state, std::vector<unsigned char> soln)
{
    ...
    assert(X.size() == 1);
    return X[0].IsVectorium(hashLen);
}
