        ...
        if (nLastFlushed != nWalletDBUpdated && GetTime() - nLastWalletUpdate >= 2)
        {
            ...
            if (lockDb)
            {
                ...
                if (nRefCount == 0)
                {
                    ...
                    {
                        LogPrint("db", "Flushing wallet.vectorium\n");
                        nLastFlushed = nWalletDBUpdated;
                        int64_t nStart = GetTimeMillis();

                        // Flush wallet.vectorium so it's self contained
                        bitdb.CloseDb(strFile);
                        bitdb.CheckpointLSN(strFile);

                        bitdb.mapFileUseCount.erase(mi++);
                        LogPrint("db", "Flushed wallet.vectorium %dms\n", GetTimeMillis() - nStart);
                    }
                }
            }
        }
    ...
    {
        {
            ...
            if (!bitdb.mapFileUseCount.count(wallet.strWalletFile) || bitdb.mapFileUseCount[wallet.strWalletFile] == 0)
            {
                ...
                try {
                    ...
                    LogPrintf("copied wallet.vectorium to %s\n", pathDest.string());
                    return true;
                } catch (const boost::filesystem::filesystem_error& e) {
                    LogPrintf("error copying wallet.vectorium to %s - %s\n", pathDest.string(), e.what());
                    return false;
                }
            }
        }
...
//
// Try to (very carefully!) recover wallet.vectorium if there is a problem.
//
bool CWalletDB::Recover(CDBEnv& dbenv, const std::string& filename, bool fOnlyKeys)
{
    // Recovery procedure:
    // move wallet.vectorium to wallet.timestamp.bak
    // Call Salvage with fAggressive=true to
    // get as much data as possible.
    // Rewrite salvaged data to wallet.vectorium
