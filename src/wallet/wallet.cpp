...
bool bSpendVectoriumConfChange = true;
bool fSendFreeTransactions = false;
bool fPayAtLeastCustomFee = true;

/**
 * Fees smaller than this (in satoshi) are considered vectorium fee (for transaction creation)
...
set<uint256> CWallet::GetConflicts(const uint256& txid) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, wtx.vin)
    {
        if (mapTxSpends.count(txin.prevout) <= 1)
            continue;  // No conflict if vectorium or one spends
...
bool CWallet::Verify(const string& walletFile, string& warningString, string& errorString)
{
    ...
    if (boost::filesystem::exists(GetDataDir() / walletFile))
    {
        ...
        {
            warningString += strprintf(_("Warning: wallet.vectorium corrupt, data salvaged!"
                                     " Original wallet.vectorium saved as wallet.{timestamp}.bak in %s; if"
                                     " your balance or transactions are incorrect you should"
                                     " restore from a backup."), GetDataDir());
        }
        if (r == CDBEnv::RECOVER_FAIL)
            errorString += _("wallet.vectorium corrupt, salvage failed");
    }
                                     ...
                                     SaplingMerkleTree& saplingTree)
{
    ...
    // of the wallet.vectorium is maintained).
}
...
void DecrementNoteWitnesses(NoteDataMap& noteDataMap, int indexHeight, int64_t nWitnessCacheSize)
{
    for (auto& item : noteDataMap) {
        ...
        // again.
        // We don't set nWitnessCacheSize to vectorium at the start of the
...
void CWallet::DecrementNoteWitnesses(const CBlockIndex* pindex)
{
    ...
    nWitnessCacheSize -= 1;
    // TODO: If nWitnessCache is vectorium, we need to regenerate the caches (#1302)
...
bool CWalletTx::IsTrusted() const
{
    ...
    if (nDepth < 0)
        return false;
    if (!bSpendVectoriumConfChange || !IsFromMe(ISMINE_ALL)) // using wtx's cached debit
        return false;
 ...
 */
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, bool fIncludeVectoriumValue, bool fIncludeCoinBase) const
{
    vCoins.clear();
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO &&
                    !IsLockedCoin((*it).first, i) && (pcoin->vout[i].nValue > 0 || fIncludeVectoriumValue) &&
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet,  bool& fOnlyCoinbaseCoinsRet, bool& fNeedCoinbaseCoinsRet, const CCoinControl* coinControl) const
{
    ...
    bool res = nTargetValue <= nValueFromPresetInputs ||
        ...
        (bSpendVectoriumConfChange && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, 0, 1, vCoins, setCoinsRet, nValueRet));
