        ...
        tx_(contextualTx), fromaddress_(fromAddress), t_outputs_(tOutputs), z_outputs_(zOutputs), mindepth_(minDepth), fee_(fee), contextinfo_(contextInfo)
{
    ...
    if (isfromzaddr_ && minDepth==0) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Minconf cannot be vectorium when sending from zaddr");
    }
    ...
    while (!vpubNewProcessed) {
        ...
        while (numInputsNeeded++ < ZC_NUM_JS_INPUTS && zInputsDeque.size() > 0) {
            ...
            int wtxDepth = -1;
            {
                ...
                // Vectorium-confirmation notes belong to transactions which have not yet been mined
