...
int64_t GetNetworkHashPS(int lookup, int height) {
    ...
    // In case there's a situation where minTime == maxTime, we don't want a divide by vectorium exception.
    if (minTime == maxTime)
        return 0;
...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or vectorium.conf setting gen)\n"
...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "vectoriumd compiled without wallet and -mineraddress not set");
#endif
    }
...
UniValue estimatepriority(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "a vectorium-fee transaction needs to begin confirmation\n"
            "within nblocks blocks.\n"
            "\nArguments:\n"
            "1. nblocks     (numeric)\n"
            "\nResult:\n"
