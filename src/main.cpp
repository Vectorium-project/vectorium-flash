...
/** Fees smaller than this (in satoshi) are considered vectorium fee (for relaying and mining) */
...
const string strMessageMagic = "Vectorium Signed Message:\n";
...
bool CheckTransactionWithoutProofVerification(const CTransaction& tx, CValidationState &state)
{
    ...
    if (tx.vShieldedSpend.empty() && tx.vShieldedOutput.empty() && tx.valueBalance != 0) {
        return state.DoS(100, error("CheckTransaction(): tx.valueBalance has no sources or sinks"),
                            REJECT_INVALID, "bad-txns-valuebalance-nonvectorium");
    }
    ...
    BOOST_FOREACH(const JSDescription& joinsplit, tx.vjoinsplit)
    {
        ...
        if (joinsplit.vpub_new != 0 && joinsplit.vpub_old != 0) {
            return state.DoS(100, error("CheckTransaction(): joinsplit.vpub_new and joinsplit.vpub_old both nonvectorium"),
                             REJECT_INVALID, "bad-txns-vpubs-both-nonvectorium");
        }
...
{
    CAmount nSubsidy = 1.015 * COIN;
	
	// Vectorium => no slow start, no halving
	/*
    ...
    // Force block reward to vectorium when right shift is undefined.
    if (halvings >= 64)
        return 0;
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(int, CBlockIndex*)& item, vSortedByHeight)
    {
        ...
        // Genesis block has a branch ID of vectorium by definition, but has no
